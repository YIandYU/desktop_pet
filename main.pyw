...
class DesktopPetApp:
    ...
    def __init__(self):
        ...
        # 启动问候（用对话气泡显示）
        self._greeting_start()

    def _greeting_start(self):
        """程序启动时用对话气泡显示欢迎问候"""
        import datetime
        now = datetime.datetime.now()
        time_str = now.strftime("%Y年%m月%d日%H时%M分")
        msg = f"现在是{time_str}，我是J.A.R.V.I.S，很高兴见到你！"
        # 使用 pet 自带的气泡系统
        self.pet.bubble.show_text(msg)
        self.pet.bubble.duration = 300  # 约5秒
        self._startup_timer = 300

    def _greeting_end(self):
        """程序退出时用对话气泡显示告别问候，等待气泡显示完才退出"""
        import datetime
        now = datetime.datetime.now()
        time_str = now.strftime("%Y年%m月%d日%H时%M分")
        msg = f"现在是{time_str}，期待我们下次再见！"
        self.pet.bubble.show_text(msg)
        self.pet.bubble.duration = 180  # 约3秒
        self._exit_bubble_timer = 180
            ...
            if sys.platform == 'win32':
                try:
                    ...
                    ctypes.windll.user32.SetWindowPos(
                        hwnd, -1, 0, 0, self.disp_w, self.disp_h, 0x0010
                    )
                    # 置顶
    ...
    def _draw_monitor_panel(self):
        ...
        title = self.mfont.render('J.A.R.V.I.S. 硬件监测', True, (220, 235, 255))
        # 标题居中：计算 x 偏移 = (面板宽度 - 文字宽度) / 2
        title_x = (pw - title.get_width()) // 2
        bg.blit(title, (title_x, 4))
            ...
            if '%' in line:
                ...
                if match:
                    try:
                        ...
                        if line.startswith('BATT:') or line.startswith('电池'):
                            # 电池：>=80% 绿色 | >=50% 黄色 | <20% 红色
                            color = (100, 255, 100) if v >= 80 else ((255, 200, 80) if v >= 50 else ((255, 100, 100) if v < 20 else (255, 200, 80)))
                        elif '盘空间' in line or '磁盘' in line or line.startswith('DISK:'):
                            # 磁盘：>80% 红色 | >50% 黄色 | <20% 绿色
                            color = (255, 100, 100) if v > 80 else ((255, 200, 80) if v > 50 else ((100, 255, 100) if v < 20 else (255, 200, 80)))
                        else:
                            # 其他（CPU/GPU/MEM/网络等）：低占用绿色
    ...
    def run(self):
        ...
        try:
            while running and not self._exit_requested:
                ...
                # ===== 更新退出气泡倒计时 =====
                if self._exit_bubble_timer > 0:
                    self._exit_bubble_timer -= 1
                ...
                self.pet.draw(self.screen)     # 绘制宠物（自带气泡绘制）
    ...
    def _shutdown(self):
        """程序退出前的清理工作：显示告别气泡后退出"""
        self._greeting_end()                    # 关闭问候气泡

        # 等待气泡显示完，同时刷新画面
        import time
        wait_frames = 0
        while self._exit_bubble_timer > 0 and wait_frames < 300:
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                    wait_frames = 999  # 强制退出
            self.pet.bubble.update()
